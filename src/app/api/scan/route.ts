...
export async function POST(request: NextRequest) {
  try {
    ...
    const { idea, audience, timeframe, mode, startupContext } = body;
    ...
    const scanMode: ScanMode = mode === 'feature' ? 'feature' : 'idea';
    const startupCtx = scanMode === 'feature' && typeof startupContext === 'string'
      ? startupContext.slice(0, 200)
      : null;

    if (scanMode === 'feature' && !startupCtx) {
      return NextResponse.json(
        { error: 'Startup context is required for feature validation.' },
        { status: 400 }
      );
    }
    ...
    const scan = await createScan(idea, audience, tf, undefined, scanMode, startupCtx);
