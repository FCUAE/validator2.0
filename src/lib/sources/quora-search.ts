...
export async function scanQuora(query: string, _audience: string, _timeframeDays: number): Promise<SourceResult> {
  try {
    const apiKey = process.env.SERPER_API_KEY;
    if (!apiKey) return createEmptyResult('quora');
    ...
    if (!response.ok) return createEmptyResult('quora');

    const data = await response.json();
    const organic = data.organic || [];

    if (organic.length === 0) return createEmptyResult('quora');
    ...
    return createEmptyResult('quora');
  }
}
